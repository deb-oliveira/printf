...
/*   Updated: 2021/07/02 19:17:09 by dde-oliv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_bytestoprint(const char *str, t_specf specf, t_print *print)
{
	if (specf.precision >= 0 && *str == 's'
		...
		print->len = specf.precision
			+ (ft_strchr(print->str, '.') - print->str) 
			+ 1;
	else if (specf.flags->hash == 1 && *str == 'g' && ft_strchr(print->str, '.') && !ft_strchr(print->str, 'e') 
			&& (ft_isdigit(*(print->str)) || ((print->str)[0] == '-' && ft_isdigit((print->str)[1]))))
		print->len = specf.precision
			+ (ft_strncmp(print->str, "0", 1) == 0 || ft_strncmp(print->str, "-0", 2) == 0 
			|| ft_strncmp(print->str, " 0", 2) == 0 || ft_strncmp(print->str, "+0", 2) == 0)
			+ 1;
	...
	else if (specf.flags->hash == 1 && *str == 'g' && !ft_strchr(print->str, '.'))
		print->len = specf.precision + 4 * (ft_strchr(print->str, 'e') != NULL);
